...
* `BPTree` is now recognized as a `SkbioObject`, so `isinstance(bp, SkbioObject)` and `issubclass(BPTree, SkbioObject)` are `True` and it participates in the scikit-bio class hierarchy like `TreeNode`. `BPTree` remains a compiled Cython extension type; the membership is established by virtual registration, at no runtime or memory cost ([#2570](https://github.com/scikit-bio/scikit-bio/pull/2570)).
* `faith_pd` and `alpha_diversity("faith_pd", ...)` now accept a `BPTree` reference tree together with an `engine` parameter: `"cython"` (the default), `"numba"`, or `"gpu"` (the latter two require the optional `numba` dependency, and `"gpu"` additionally a CUDA or ROCm device, falling back to `"numba"` when none is usable). For a `BPTree`, Faith's PD uses an array-native reformulation: because a node's descendant tips occupy a contiguous run in tip order, the post-order `counts_by_node` reduction becomes a per-sample present-taxon prefix-sum plus a range test, evaluated in one batched, sample-parallel kernel. This never materializes the `(n_samples, n_nodes)` `counts_by_node` matrix — 26 GB for 8,192 samples against a 200,000-tip tree — and replaces the per-sample Python loop in `alpha_diversity`. All three engines match the `TreeNode` result; the `"cython"` and `"numba"` kernels are bit-identical to it regardless of thread count, and the GPU's differing summation order keeps it within one ULP. `"gpu"` must be requested per call and is not a valid `set_config("engine", ...)` value, because whether its host-to-device transfer pays depends on the shape of the workload. An explicitly requested non-`cython` engine on a `TreeNode` warns and falls back ([#XXXX](https://github.com/scikit-bio/scikit-bio/pull/XXXX)).
* `phydiv` and `alpha_diversity("phydiv", ...)` now accept a `BPTree` reference tree together with an `engine` parameter (`"cython"`, the default, or `"numba"`, which requires the optional `numba` dependency), extending the array-native acceleration added for `faith_pd` to generalized phylogenetic diversity. All four `phydiv` modes — rooted or unrooted, unweighted or abundance-weighted (including the balance-weighted `θ` parameter) — reduce to a per-node test or weight over the tree's contiguous tip-range arrays, evaluated in one batched, sample-parallel kernel. This never materializes the `(n_samples, n_nodes)` `counts_by_node` matrix and replaces the per-sample Python loop in `alpha_diversity`. Both engines are bit-identical to the `TreeNode` result regardless of thread count (non-integer counts are truncated to integers, matching the `TreeNode` path). A GPU engine for `phydiv` is deferred; an explicitly requested non-`cython` engine on a `TreeNode` warns and falls back ([#XXXX](https://github.com/scikit-bio/scikit-bio/pull/XXXX)).
...
* Sped up `BPTree` construction by building the `select` index in a single linear pass (`numpy.flatnonzero` over each parenthesis type) instead of an O(n log n) `numpy.unique` over cumulative ranks. The index build drops roughly tenfold — about a 7% reduction in total tree construction time on large trees, inherited by `parse_newick` and `from_treenode` — while producing bit-for-bit identical arrays and an unchanged memory footprint ([#2570](https://github.com/scikit-bio/scikit-bio/pull/2570)).
* Sped up `TreeNode.from_bptree` by performing the balanced-parentheses traversal in a single compiled pass instead of calling `BPTree` accessors once per node across the Python/C boundary. The traversal now returns per-node name, length, edge, and parent arrays in preorder, leaving the Python side to do only `TreeNode` assembly. Conversion is about 1.05–1.15× faster across balanced, caterpillar, and random topologies from a thousand up to a million tips, with byte-identical output ([#2570](https://github.com/scikit-bio/scikit-bio/pull/2570)).
