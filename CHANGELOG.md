...
* `faith_pd` and `alpha_diversity("faith_pd", ...)` now accept a `BPTree` reference tree together with an `engine` parameter (`"cython"`, the default, or `"numba"`, which requires the optional `numba` dependency). For a `BPTree`, Faith's PD uses an array-native reformulation: because a node's descendant tips occupy a contiguous run in tip order, the post-order `counts_by_node` reduction becomes a per-sample present-taxon prefix-sum plus a range test, evaluated in one batched, sample-parallel kernel. This never materializes the `(n_samples, n_nodes)` `counts_by_node` matrix and replaces the per-sample Python loop in `alpha_diversity`. Results match the `TreeNode` path exactly; a non-`cython` engine requested for a `TreeNode` warns and falls back to the cython path ([#XXXX](https://github.com/scikit-bio/scikit-bio/pull/XXXX)).
